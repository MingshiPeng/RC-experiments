...
class CBT_QC(RCDataset):
    ...
    def next_batch_feed_dict_by_dataset(self, dataset, _slice, samples):
        data = {
            ...
            "ques_doc_bt:0": dataset[4][_slice]
        }
        return data, samples
    ...
    def read_cbt_data(self, file, max_count=None):
        ...
        qd_lens = [len(i) for i in ques_docs]
        ...
        avg_qd_len = reduce(lambda x, y: x + y, qd_lens) / len(ques_docs)
        logger("Ques_doc average length: %d." % avg_q_len)
        logger("Ques_doc midden length: %d." % len(sorted(ques_docs, key=len)[len(ques_docs) // 2]))

        return documents, questions, answers, candidates, ques_docs

    def preprocess_input_sequences(self, data):
        """
        preprocess,pad to fixed length.
        """
        documents, questions, answer, candidates, ques_docs = data
        ...
        ques_docs_ok = pad_sequences(ques_docs, maxlen=self.qd_len, dtype="int32", padding="post", truncating="post")
        y_true = np.zeros_like(candidates_ok)
        y_true[:, 0] = 1
        return questions_ok, documents_ok, candidates_ok, y_true, ques_docs_ok
    ...
    def get_data_stream(self):
        ...
        self.qd_len = get_max_length(self.train_data[4])
        ...
        return self.d_len, self.q_len, self.train_sample_num, self.valid_sample_num, self.test_sample_num, self.qd_len
